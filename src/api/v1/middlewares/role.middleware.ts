...
const checkRole = async (
  ...
  try {
    ...
    if (res.locals.user.role !== role)
      return throwError(req, res, 'Unauthorized to access this endpoint', 403)
    switch (role) {
      case 'user':
        const validUser = await User.findById(res.locals.user.id)
        if (!validUser || validUser.token === null)
          return throwError(req, res, 'Invalid token credentials', 403)
        break
      case 'busOwner':
        const validBusOwner = await BusOwner.findById(res.locals.user.id)
        if (!validBusOwner || validBusOwner === null)
          return throwError(req, res, 'Invalid token credentials', 403)
        break
      default:
  ...
  try {
    ...
    if (!token)
      return throwError(
        req,
        res,
        'Missing auth token, try logging in again',
        401,
      )
    ...
    if (typeof decoded !== 'object')
      return throwError(req, res, 'Invalid Token', 401)

    if (!decoded.role || !decoded.id)
      return throwError(req, res, 'Invalid Token, Try loggin in again', 401)

...
const validateBusRegister = [
    ...
    body("busSeats").optional().isNumeric().withMessage("Bus seats must be a number")
];

const validateUserRegister = [
    body("username").notEmpty().withMessage("Username is required"),
    body("email").notEmpty().isEmail().withMessage("Email is required"),
    body("phoneNumber").notEmpty().isMobilePhone("ne-NP").withMessage("Phone number is required"),
    body("citizenshipNumber").notEmpty().withMessage("Citizenship number is required"),
    body("password").isLength({ min: 5 }).isAlphanumeric().withMessage("Password must be alphanumeric and at least 5 characters long")
];
